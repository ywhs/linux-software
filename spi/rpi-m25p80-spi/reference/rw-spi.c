...
/* spi读取fpga操作 */
int spi_read_data(uint32_t read_start_address, uint32_t read_end_address){

	// 读取数据的数组
	uint8_t read_data[BUF_LEN] = {0x00};
	// 读取的个数
	int n = 0, ret = 0;
	clm25p80 m25p80;
	...
	// 判断起始地址是否大于结束地址
	if(read_start_address > read_end_address){
		printf("Start address needs to be less than end address! \n");
		return FAILE;
	}
	...
	while( read_start_address < read_end_address){
		...
		n = read_end_address - read_start_address < BUF_LEN ? (read_end_address - read_start_address) : BUF_LEN;
		// 从fpga中读取
		if( m25p80.read( read_start_address, read_data, n ) < 0 ){
			printf("read err \n");
			return FAILE;
		}
		read_start_address += n;
	...
	for( int i = 0; i < read_end_address; i++ ){
		if (!(i % 5))
			puts("");
...
int spi_get_id(){
	...
	// 获取设备id
	if( (ret = m25p80.getId(read_id_buff)) < 0 ){
		printf("read id err, ret = %d \n", ret);
		return FAILE;
	}
...
void Usage(){
	...
	printf("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
	printf("\n");
	printf(" -r [start_address] [end_address] read data from start address to end address \n");
	printf(" -f [file_path] write file content to fpga \n");
	printf(" -w [file_path] write fpga content to file \n");
	printf(" -i get fpga device id \n");
	printf(" -c [source_filepath] [target_filepath] Compare if the contents of two files are the same \n");
	printf(" -h: help \n");
	printf("\n");
...
int main(int argc, char * argv[]){
	...
	// 定义开始地址和结束地址
	uint32_t read_start_address,read_end_address;
	...
	/* 获取脚本参数，根据不同参数执行不同操作 */
	while((ret = getopt(argc,argv,"r:f:w:ihc:")) != -1){
		switch(ret){
			case 'r':

				flag = true;
				read_start_address = atoi(optarg);
				
				// 判断是否跟着结束地址
				if(argc != 4){
					printf("missing end address, ./dl-spi -r 0 10 \n");
					exit(1);
				}
				read_end_address = atoi(argv[optind]);

				printf("read_start_address: %d \n", read_start_address);
				printf("read_end_address: %d \n", read_end_address);

				spi_read_data(read_start_address, read_end_address);
				break;
			case 'f':
				flag = true;
			...
			case 'w':
				flag = true;
				spi_read_data_to_file(optarg, 0, 341160);
				break;
			case 'i':
				flag = true;
				spi_get_id();
			...
			case 'c':
				...
				// 判断是否有目标文件
				if(argc != 4){
					printf("missing end target_file, ./dl-spi -r source_filename target_filename \n");
					exit(1);
				}
				file_compare(optarg, argv[optind]);
				break;
			case 'h':
				Usage();
				break;
